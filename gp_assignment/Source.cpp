...
float pry = 0, prSpeed = 3; //rotate ry for projection
...
//Ambient Light
float amb[3] = { 1, 1, 1 }; //Ambient Light - White
float posA[3] = { posX , posY, posZ }; //Ambient light Position

//Ambient Light - Material
float ambMBlue[3] = { 0, 0, 1 }; //Ambient light Material - Blue
float ambMRed[3] = { 1, 0, 0 }; //Ambient light 2 Material - Red
float ambMYellow[3] = { 1, 1, 0 }; //Ambient light 2 Material - Yellow
float ambMGrey[3] = { 0.5, 0.5, 0.5 }; //Ambient light 2 Material - Grey
float ambMWhite[3] = { 1, 1, 1 }; //Ambient light 2 Material - White

//Diffuse Light
float dif[3] = { 1, 1, 1 }; //Diffuse light - White
float posD[3] = { posX , posY, posZ }; //Diffuse light Position

//Diffuse Light - Material
float difMBlue[3] = { 0, 0, 1 }; //Diffuse light Material - Blue
float difMRed[3] = { 1, 0, 0 }; //Diffuse light 2 Material - Red
float difMYellow[3] = { 1, 1, 0 }; //Diffuse light 2 Material - Yellow
float difMGrey[3] = { 0.5, 0.5, 0.5 }; //Diffuse light 2 Material - Grey
float difMWhite[3] = { 1, 1, 1 }; //Diffuse light 2 Material - White
...
float movefleft2 = 0, movefright2 = 0;
...
LRESULT WINAPI WindowProcedure(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		else if (wParam == 'T') {                 //change texture
			change = change + 1;
			if (change > 2) {
				change = 0;
			}
		...
		//Walking Animation
		else if (wParam == 'M') {		//Move Front
			movefront = true;
		}
		else if (wParam == 'N') {		//Move Back
			movefront = false;
		}
		...
		else if (wParam == 'Z') {								//Switch Ambient Light
			changeLight = 1;
		}
		else if (wParam == 'X') {								//Swithc Diffuse Light
			changeLight = 2;
		}
...
void projection() {
	...
	else
	{
		...
		glFrustum(-25, 25, -20, 20, PNear, PFar); //xmin, xmax, ymin, ymax, -zmin(need to start from positive), -zmax
...
void lighting()
{
	...
	if (changeLight == 1)
	{
		...
		glMaterialfv(GL_FRONT, GL_AMBIENT, ambMRed);
		glMaterialfv(GL_FRONT, GL_AMBIENT, ambMBlue);
		glMaterialfv(GL_FRONT, GL_AMBIENT, ambMYellow);
		glMaterialfv(GL_FRONT, GL_AMBIENT, ambMGrey);
	...
	else if (changeLight == 2)
	{
		...
		glMaterialfv(GL_FRONT, GL_DIFFUSE, difMBlue);
		glMaterialfv(GL_FRONT, GL_DIFFUSE, difMRed);
		glMaterialfv(GL_FRONT, GL_DIFFUSE, difMYellow);
		glMaterialfv(GL_FRONT, GL_DIFFUSE, difMGrey);
...
void robot_rightHand() {
	...
	//-------------------- ROBOT MOVING HAND WHEN WALKING --------------------
	if (isWalking == true)
	{
		handMovementSpeed = 1;
		right_hand_x = -1;
...
void robotLeg(float movef, float movef2) {
	glPushMatrix();
	///////////////////////////////////////////////////////////////////////////
	...
	glTranslatef(-1.2, 4.6, 0.0);

	glRotatef(-(movef2), 1.0, 0.0, 0.0);
	...
	glRotatef(90, 0.0, 1.0, 0.0);
	textureArr[1] = loadTexture("tire.bmp");
	drawFillCoverCylinder(0.7, 1, 10, 10);
	glDeleteTextures(1, &textureArr[1]);
...
void background() {
	//glTranslatef(-25.0, -25.0, -5.0);
	GLuint textureArr[1];
	textureArr[0] = loadTexture("sky.bmp");
	glColor3f(1.0, 1.0, 1.0);
	//drawFillCube(50.0, 50.0, 50.0);
...
void display()
{
	...
	glClearColor(0.53, 0.8, 0.9, 0.0f);
	...
	if (isWalking && movefront) {
		...
		if (walk >= 2)
		{
			...
			movefleft = 0, movefright = 0;
			movefleft2 = 0, movefright2 = 0;
		}
	}
	else if (isWalking && !movefront) {
		walk -= walkspeed;
	...
	robotLeg(movefleft, movefleft2); //Robot leg
	glPopMatrix();
	...
	glScalef(0.5, 0.35, 0.5);
	robotLeg(movefright, movefright2); //Robot leg
	glPopMatrix();
